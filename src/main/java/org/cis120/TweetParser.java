...
public class TweetParser {
    ...
    static String extractColumn(String csvLine, int csvColumn) {
        ...
        try {
            arrOfString = csvLine.split(",");
            if (arrOfString[csvColumn].isEmpty()) {
                return null;
            } else {
                return arrOfString[csvColumn];
            }
    ...
    static List<String> csvDataToTweets(BufferedReader br, int tweetColumn) {
        ...
        FileLineIterator fileIter = new FileLineIterator(br);
        try {
            while (fileIter.hasNext()) {
                String newWord = fileIter.next();
                if (newWord.length() > 0 && newWord != null) {
                    tweets.add(extractColumn(newWord, tweetColumn));
                }
            }
    ...
    static List<String> parseAndCleanSentence(String sentence) {
        ...
        String [] arrOfString = sentence.split(" ");
        for (int i = 0; i < arrOfString.length; i++) {
            String cleanWord = cleanWord(arrOfString[i]);
            if (cleanWord != null && !(cleanWord.isEmpty())) {
                cleanSentence.add(cleanWord);
            }
        }
    ...
    static List<List<String>> parseAndCleanTweet(String tweet) {
        ArrayList<List<String>> tweetList = new ArrayList<List<String>>();
        String cleanTweet = removeURLs(tweet);
        List<String> sentences = tweetSplit(cleanTweet);
        for (int i = 0; i < sentences.size(); i++) {
            List<String> cleanSentences = parseAndCleanSentence(sentences.get(i));
            if (cleanSentences != null && !(cleanSentences.isEmpty())) {
                tweetList.add(cleanSentences);
            }
        }
        return tweetList;
    }
    ...
    public static List<List<String>> csvDataToTrainingData(BufferedReader br, int tweetColumn) {
        List<String> data = csvDataToTweets(br, tweetColumn);
        List<List<String>> trainingData = new LinkedList<>();
        for (int i = 0; i < data.size(); i++) {
            if (data.get(i) != null) {
                List<List<String>> cleanTweet = parseAndCleanTweet(data.get(i));
                if (cleanTweet.size() != 0) {
                    trainingData.addAll(cleanTweet);
                }
            }
        }
        return trainingData;
