...
public class TwitterBot {
    ...
    public TwitterBot(BufferedReader br, int tweetColumn, NumberGenerator ng) {
        ...
        this.ng = ng;
        List<List<String>> tweetData = TweetParser.csvDataToTrainingData(br, tweetColumn);
        for (List<String> tweet : tweetData) {
    ...
    public void writeStringsToFile(
        ...
        try {
            ...
            for (int i = 0; i < stringsToWrite.size(); i++) {
                bw.write(stringsToWrite.get(i), 0 , stringsToWrite.get(i).length());
                bw.newLine();
            }
        } catch (IOException e) {
            e.printStackTrace();
        ...
        if (numWords < 1) {
            ...
            while (counter <= numWords) {
                if (!(mc.hasNext())) {
                    ...
                    String newWord = mc.next();
                    if (counter + 1 >= numWords) {
                        tweet = tweet + " " + newWord + randomPunctuation();
                        break;
                    } else {
                        tweet = tweet + " " + newWord;
                    }
            ...
            return tweet.substring(1, tweet.length());
        }
        return tweet;
    }
